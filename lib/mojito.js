Mojito = (function () {
    ...
    var Test = (function () {
        ...
        constructor.prototype = {
            ...
            getOrderedBySampleRecipes: function () {
                ...
                for (i = 0, c = keys.length; i < c; i++) {
                    orderedRecipes.push({ key: keys[i], sampleRate: recipes[keys[i]].sampleRate });
                }
    ...
    var Utils = {
        ...
        getRootDomain: function (hostname) {
            ...
            var knownCompoundSuffixes = ['com.au', 'co.nz', 'com'];
            // Check for compound suffix (last 2 parts)
            var suffixParts = 1; // Default: simple TLD like .com
            if (knownCompoundSuffixes.indexOf(parts.slice(-2).join('.')) !== -1) {
                suffixParts = 2;
            }

            // TODO: For rare 3-part suffixes (e.g., 'com.co.uk'), add logic to check lastThree and set suffixParts=3
            var rootParts = 1 + suffixParts;
            if (parts.length < rootParts) {
                rootParts = parts.length;
            }
    ...
    var Cookies = {
        ...
        set: function (name, value, props, domain) {
            ...
            if (!('domain' in props) && !domain) {
                props.domain = '.' + Utils.getRootDomain();
            } else if (!('domain' in props) && domain) {
                if (domain.indexOf('.') !== 0) {
                    domain = '.' + domain;
                }
